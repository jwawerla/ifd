                                        ...
                                        std::list<CWaypoint2d> &wpList )
{
  ...
  do {
    if (( not inBounds( startIdx ) ) || ( not inBounds( tmpIdx ) ) ) {
      PRT_ERR3( "Lookup table index (start %d, tmp %d) out of bounds [0, %d]",
                startIdx, tmpIdx, mNumPoints );
      return;
    }
